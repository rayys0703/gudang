<x-app-layout>
    <x-slot name="header">
        ...
        <div class="flex items-center gap-x-5">
            {{-- <form action="{{ route('supplier.index') }}" method="GET" class="flex items-center max-w-sm mx-auto">
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-5">
                ...
                <table id="supplier-table" class="py-6 px-0 w-full text-sm text-left rtl:text-right text-gray-500">
                    <thead class="text-xs text-gray-900 uppercase bg-white">
                        <tr>
                            <th scope="col" class="px-6 py-3"><input type="checkbox" id="select-all"></th>
                ...
                <script>
                    $(document).ready(function() {
                        $('#supplier-table').DataTable({
                            ...
                            columns: [{
                                    data: 'id',
                                    orderable: false, // Nonaktifkan fungsi pengurutan di kolom checkbox
                                ...
                                {
                                    data: 'id',
                                    orderable: false, // Nonaktifkan fungsi pengurutan di kolom aksi
                                    render: function(data, type, row) {
                                        return `
                                            <div class="flex">
                                                <a href="/supplier/edit/${data}" class="flex text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-2xl text-sm px-3 py-1.5 text-center mr-2" type="button">
                                                ...
                                                <a href="/supplier/delete/${data}" class="flex text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-2xl text-sm px-3 py-1.5 text-center" type="button">
                ...
                {{-- <div class="flex space-x-2 my-4">
                    @if (isset($pagination['links']))
                        @foreach ($pagination['links'] as $link)
                            @if ($link['active'])
                                <span
                                    class="px-3 py-2 bg-blue-500 text-white font-semibold rounded-md">{{ $link['label'] }}</span>
                            @else
                                <a href="?{{ explode('?', $link['url'])[1] ?? '' }}"
                                    class="px-3 py-2 bg-white text-blue-500 font-semibold rounded-md border border-blue-500 hover:bg-blue-100 transition duration-150 ease-in-out">{{ $link['label'] }}</a>
                            @endif
                        @endforeach
                    @endif
                </div> --}}
