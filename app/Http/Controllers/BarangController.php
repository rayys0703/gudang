...
class BarangController extends Controller
{
	...
	{
		$response = Http::withToken(session('token'))->get(config('app.api_url') . '/barang/create');
	...
	{
		$response = Http::withToken(session('token'))->post(config('app.api_url') . '/barang', $request->all());
	...
	{
		$response = Http::withToken(session('token'))->get(config('app.api_url') . '/barang/' . $id);
	...
	{
		$response = Http::withToken(session('token'))->put(config('app.api_url') . '/barang/' . $id, $request->all());
	...
	{
		$response = Http::withToken(session('token'))->delete(config('app.api_url') . '/barang/' . $id);
	...
	{
		$response = Http::withToken(session('token'))->post(config('app.api_url') . '/barang/delete-selected', [
